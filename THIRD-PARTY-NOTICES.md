# Third-Party Notices

This project uses third-party libraries and services. Below is a list of all third-party dependencies used in this project, along with their licenses and copyright information.

---

## PHP Dependencies (via Composer)

### 1. PHPDotenv
- **Package**: `vlucas/phpdotenv`
- **Version**: ^5.6
- **License**: BSD-3-Clause
- **Copyright**: Copyright (c) 2013, Vance Lucas
- **Repository**: https://github.com/vlucas/phpdotenv
- **Purpose**: Loads environment variables from `.env` file into `getenv()`, `$_ENV`, and `$_SERVER`
- **License Text**: https://github.com/vlucas/phpdotenv/blob/master/LICENSE

### 2. Cloudinary PHP SDK
- **Package**: `cloudinary/cloudinary_php`
- **Version**: ^3.1
- **License**: MIT License
- **Copyright**: Copyright (c) 2012-2024 Cloudinary
- **Repository**: https://github.com/cloudinary/cloudinary_php
- **Purpose**: Cloud-based image and video management service integration
- **License Text**: https://github.com/cloudinary/cloudinary_php/blob/master/LICENSE

---

## Frontend Libraries (CDN)

### 3. Bootstrap
- **Version**: 5.3.x
- **License**: MIT License
- **Copyright**: Copyright (c) 2011-2024 The Bootstrap Authors
- **Website**: https://getbootstrap.com/
- **Purpose**: Responsive CSS framework for building modern web interfaces
- **License Text**: https://github.com/twbs/bootstrap/blob/main/LICENSE

### 4. Font Awesome
- **Version**: 6.x
- **License**: 
  - Icons: CC BY 4.0 License
  - Fonts: SIL OFL 1.1 License
  - Code: MIT License
- **Copyright**: Copyright (c) Fonticons, Inc.
- **Website**: https://fontawesome.com/
- **Purpose**: Icon library for web applications
- **License Text**: https://fontawesome.com/license/free

### 5. jQuery (if used)
- **Version**: 3.x
- **License**: MIT License
- **Copyright**: Copyright OpenJS Foundation and other contributors
- **Website**: https://jquery.com/
- **Purpose**: JavaScript library for DOM manipulation
- **License Text**: https://jquery.org/license/

---

## Third-Party Services

### 6. Cloudinary Service
- **Service**: Cloud-based image and video management
- **Website**: https://cloudinary.com/
- **Purpose**: Image storage, optimization, and delivery
- **Terms of Service**: https://cloudinary.com/tos
- **Privacy Policy**: https://cloudinary.com/privacy

### 7. Gmail SMTP (Optional)
- **Service**: Google Gmail SMTP for email delivery
- **Website**: https://mail.google.com/
- **Purpose**: Sending transactional emails (verification, password reset)
- **Terms of Service**: https://policies.google.com/terms
- **Privacy Policy**: https://policies.google.com/privacy

---

## Development Dependencies

### 8. Composer
- **Tool**: Dependency Manager for PHP
- **License**: MIT License
- **Website**: https://getcomposer.org/
- **Purpose**: Managing PHP package dependencies
- **License Text**: https://github.com/composer/composer/blob/main/LICENSE

---

## Database

### 9. MySQL
- **Database**: MySQL Community Server
- **Version**: 8.x
- **License**: GPL v2 (with FOSS License Exception)
- **Copyright**: Copyright (c) 2000, 2024, Oracle Corporation and/or its affiliates
- **Website**: https://www.mysql.com/
- **Purpose**: Relational database management system
- **License Text**: https://www.gnu.org/licenses/old-licenses/gpl-2.0.html

---

## Optional Services (Prepared but not fully implemented)

### 10. Redis (Optional)
- **Service**: In-memory data structure store
- **License**: BSD-3-Clause
- **Website**: https://redis.io/
- **Purpose**: Caching layer for improved performance
- **License Text**: https://redis.io/docs/about/license/

### 11. Supabase (Optional)
- **Service**: Open source Firebase alternative
- **License**: Apache License 2.0
- **Website**: https://supabase.com/
- **Purpose**: Alternative storage and backend services
- **License Text**: https://github.com/supabase/supabase/blob/master/LICENSE
